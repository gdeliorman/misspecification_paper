...
##ICA grid={-1, -0.975,-0.925...} 81^4=43046721 90727 pdf matrices
ICA<-ICA.ContCont(cT0S0, cT1S1, vT0T0, vT1T1, vS0S0, vS1S1, T0T1=seq(-1, 1, by=.025), 
                  T0S1=seq(-1, 1, by=.025), T1S0=seq(-1, 1, by=.025), S0S1=seq(-1, 1, by=.025))
...
hist(Ica_normal, xlim = c(0,1), ylim=c(0,45000), main=bquote("Histogram of ICA"[N]), xlab=bquote("ICA"[N]))
grid()
box()
...
Ica_t<- ICA_t(
  ...
  S0S0 = vS0S0,
  S1S1 = vS1S1,
  T0T1 = seq(-1, 1, by = 0.025),
  T0S1 = seq(-1, 1, by = 0.025),
  T1S0 = seq(-1, 1, by = 0.025),
  S0S1 = seq(-1, 1, by = 0.025)
)
...
hist(Ica_t_dist, xlim = c(0,1), ylim=c(0,45000), main=bquote("Histogram of ICA"[t]), xlab=bquote("ICA"[t]))
grid()
box()
...
write.table(outputs, file="log_normal_output_newgrid_29sep.txt",  sep = "\t", row.names = FALSE, col.names = FALSE, append=TRUE)
...
data_lognormal_newgrid<- read.table(file="log_normal_output_newgrid_29sep.txt")
ica_log_normal_newgrid<- as.numeric(unlist(data_lognormal_newgrid[4]))
plot(density(ica_log_normal_newgrid))
summary(ica_log_normal_newgrid)
...
##ICA normal with wider grid
save(Ica_normal,file = "Ica_normal_schizo_newgrid.rda")
...
##ICA_t with wider grid
save(Ica_t_dist, file = "Ica_t_schizo_newgrid.rda")
...
##ICA log-normal with wider grid
save(ica_log_normal_newgrid, file = "Ica_lognormal_schizo_newgrid.rda")
